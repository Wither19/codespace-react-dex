...
function Stats(props) {
  ...
  function updatePOGOValues() {
    ...
    if (props.statList) {
      ...
      var scaledAtk = Math.round(
        2 *
          ((7 / 8) * Math.max(props.statList[1].base_stat, props.statList[3].base_stat) +
            (1 / 8) * Math.min(props.statList[1].base_stat, props.statList[3].base_stat))
      );
      var scaledDef = Math.round(
        2 *
          ((5 / 8) * Math.max(props.statList[2].base_stat, props.statList[4].base_stat) +
            (3 / 8) * Math.min(props.statList[2].base_stat, props.statList[4].base_stat))
      );
  ...
  useEffect(updatePOGOValues, [props.statList, attackIV, defenseIV, hpIV, level]);
      ...
      <div style={{ display: pogoDisplay ? "none" : "block" }}>
        ...
        {props.statList &&
          props.statList.map((stat, index) => (
            <Stat key={index} statName={stat.stat.name} statNumber={stat.base_stat} />
          ))}
      </div>
        ...
        <div>
          ...
          >
            ...
            >
              ...
              <IVInput stat="def" iv={defenseIV} changeFunc={updatePOGOValues} />
          ...
          <center>
            <FaStar
              size="3em"
              color={
                ivTotal >= 24 && ivTotal != 45 ? "#d49831" : ivTotal >= 45 ? "#ba3838" : "lightgray"
              }
            ...
            <FaStar
              size="3em"
              color={
                ivTotal >= 31 && ivTotal != 45 ? "#d49831" : ivTotal >= 45 ? "#ba3838" : "lightgray"
              }
              style={{ margin: "2px 6px" }}
            />
            <FaStar
              size="3em"
              color={
                ivTotal > 38 && ivTotal != 45 ? "#d49831" : ivTotal >= 45 ? "#ba3838" : "lightgray"
              }
              style={{ margin: "2px 6px" }}
            />
          </center>
